...
export const getUserByUsername = async (req, res) => {
  ...
  // Query to the database
  const [user] = await connection.query("SELECT * FROM users WHERE username = ?", [req.params.username]);
...
export const getUserByEmail = async (req, res) => {
  ...
  // Query to the database
  const [user] = await connection.query("SELECT * FROM users WHERE email= ?", [req.params.email]);
...
export const createUser = async (req, res) => {
  try {
    ...
    const usersByUsername = await findUserByUsername(req.body.username, connection);
    ...
    const [results] = await connection.query("INSERT INTO users (name, email, username, password) VALUES (?, ?, ?, ?)", [req.body.name, req.body.email, req.body.username, req.body.password]);
...
export const updateUser = async (req, res) => {
  ...
  // Query to the database
  const result = await connection.query("UPDATE users SET ? WHERE id = ?", [req.body, req.params.id]);
